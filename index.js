...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:Causs\n' // Nama kamu
            + 'ORG:Lexa;\n' // Nama bot
            + 'TEL;type=CELL;type=VOICE;waid=5573988453021:+55 73988453021\n' //Nomor whatsapp kamu
            + 'END:VCARD'
prefix = '.','!','#'
blocked = []
limitawal = '999999999'
		...
		try {
			...
			mess = {
				...
				only: {
					...
					mod: '[❗] ESTE PEDIDO É ESPECÍFICO PARA USUARIO MOD BIXCOITO BOT*',
					benned: 'Você para a banda, por favor, contate o proprietário para abrir sua banda',
					ownerG: '[❗] Causs? Este é um recurso especial para o Bixcoito ❌',
					ownerB: '[❗] Causs? Este é um recurso especial para o Bixcoito❌',
					userB: `──「 LISTA 」──\nOlá Kak !\nDesculpe, irmã. Você não está registrado como amigo de BIXCOITO. Registre-se para fazer amizade com o bot Bixcoito por meio, \n\nCommand : ${prefix}daftar nama|idade\nExemplo : ${prefix}daftar Bixcoito|17\n\n──「 Bixcoito BOT 」──`,
			...
			const ownerNumber = ["5573988453021@s.whatsapp.net"] // replace this with your number
			const mod = [ownerNumber,"5573988453021@s.whatsapp.net"]//ubah nomor lo
			const adminbotnumber = ["5573988453021@s.whatsapp.net"]//ubah nomor lo
			const frendsowner = ["5573988453021@s.whatsapp.net"]//ubah nomor lo
			const premium = ["5573988453021@s.whatsapp.net","5573988453021@s.whatsapp.net"] 
...
if (text.includes("placa"))
  ...
  conn.sendMessage(id, hasil, MessageType.text); 
		...
		if (!isGroup) return
		if (!isAntiLink) return
		if (isGroupAdmins) return reply('vc é admin, então n irei te dar ban por usar links, rlx puta 🙂')
		client.updatePresence(from, Presence.composing)
		if (messagesC.includes("#izinadmin")) return reply("#izinadmin diterima")
		var kic = `${sender.split("@")[0]}@s.whatsapp.net`
		reply(`link detectado ${sender.split("@")[0]} voce sera expulso deste grupo em 5 segundos`)
